package report

import (
	"bufio"
	"encoding/json"
	"fmt"
	"os"
	"strings"

	"aero-rig/internal/probe"
)

const ChatStreamSummarySchemaV1 = "aerorig.chat_stream_summary.v1"

type ChatStreamSummary struct {
	SchemaVersion      string          `json:"schema_version"`
	Probe              string          `json:"probe"`
	SourceFiles        []string        `json:"source_files"`
	TotalSamples       int             `json:"total_samples"`
	OKSamples          int             `json:"ok_samples"`
	FailedSamples      int             `json:"failed_samples"`
	SuccessRate        float64         `json:"success_rate"`
	TTFTMS             LatencySummary  `json:"ttft_ms"`
	DurationMS         LatencySummary  `json:"duration_ms"`
	SSEEvents          IntSummary      `json:"sse_events"`
	ContentChunks      IntSummary      `json:"content_chunks"`
	PromptTokens       IntSummary      `json:"prompt_tokens"`
	OutputTokens       IntSummary      `json:"output_tokens"`
	TotalTokens        IntSummary      `json:"total_tokens"`
	AnswerHashCount    int             `json:"answer_hash_count"`
	AnswerStable       bool            `json:"answer_stable"`
	CacheResults       []CountByString `json:"cache_results"`
	Tiers              []CountByString `json:"tiers"`
	VerifiedSamples    int             `json:"verified_samples"`
	VerifiedHitSamples int             `json:"verified_hit_samples"`
	StatusCodes        []CountByStatus `json:"status_codes"`
	FinishReasons      []CountByString `json:"finish_reasons"`
	ResponseModels     []CountByString `json:"response_models"`
	Errors             []CountByString `json:"errors"`
}

func SummarizeChatStream(paths []string) (ChatStreamSummary, error) {
	s := ChatStreamSummary{
		SchemaVersion: ChatStreamSummarySchemaV1,
		Probe:         "chat_stream_summary",
		SourceFiles:   paths,
	}

	var ttfts []float64
	var durations []float64
	var sseEvents []int
	var contentChunks []int
	var promptTokens []int
	var outputTokens []int
	var totalTokens []int

	statusCounts := map[int]int{}
	errorCounts := map[string]int{}
	finishReasonCounts := map[string]int{}
	responseModelCounts := map[string]int{}
	cacheCounts := map[string]int{}
	tierCounts := map[string]int{}
	answerHashes := map[string]int{}

	for _, path := range paths {
		if strings.TrimSpace(path) == "" {
			continue
		}

		if err := readChatStreamResults(path, func(r probe.ChatStreamResult) {
			s.TotalSamples++

			if r.DurationMS > 0 {
				durations = append(durations, r.DurationMS)
			}
			if r.TTFTMS > 0 {
				ttfts = append(ttfts, r.TTFTMS)
			}
			if r.SSEEvents > 0 {
				sseEvents = append(sseEvents, r.SSEEvents)
			}
			if r.ContentChunks > 0 {
				contentChunks = append(contentChunks, r.ContentChunks)
			}
			if r.StatusCode > 0 {
				statusCounts[r.StatusCode]++
			}

			if r.OK {
				s.OKSamples++

				if r.PromptTokens > 0 {
					promptTokens = append(promptTokens, r.PromptTokens)
				}
				if r.OutputTokens > 0 {
					outputTokens = append(outputTokens, r.OutputTokens)
				}
				if r.TotalTokens > 0 {
					totalTokens = append(totalTokens, r.TotalTokens)
				}
				if strings.TrimSpace(r.AnswerSHA256) != "" {
					answerHashes[r.AnswerSHA256]++
				}
				if strings.TrimSpace(r.FinishReason) != "" {
					finishReasonCounts[r.FinishReason]++
				}
				if strings.TrimSpace(r.ResponseModel) != "" {
					responseModelCounts[r.ResponseModel]++
				}
			} else {
				s.FailedSamples++
				errText := strings.TrimSpace(r.Error)
				if errText == "" {
					errText = "unknown error"
				}
				errorCounts[errText]++
			}

			cache := getAeroHeader(r.AeroHeaders, "X-Aero-Cache")
			verified := getAeroHeader(r.AeroHeaders, "X-Aero-Verified")

			if cache != "" {
				cacheCounts[cache]++
			}
			if tier := getAeroHeader(r.AeroHeaders, "X-Aero-Tier"); tier != "" {
				tierCounts[tier]++
			}
			if strings.EqualFold(verified, "true") {
				s.VerifiedSamples++
				if strings.EqualFold(cache, "hit") {
					s.VerifiedHitSamples++
				}
			}
		}); err != nil {
			return s, err
		}
	}

	if s.TotalSamples > 0 {
		s.SuccessRate = float64(s.OKSamples) / float64(s.TotalSamples)
	}

	s.TTFTMS = summarizeDurations(ttfts)
	s.DurationMS = summarizeDurations(durations)
	s.SSEEvents = summarizeInts(sseEvents)
	s.ContentChunks = summarizeInts(contentChunks)
	s.PromptTokens = summarizeInts(promptTokens)
	s.OutputTokens = summarizeInts(outputTokens)
	s.TotalTokens = summarizeInts(totalTokens)
	s.AnswerHashCount = len(answerHashes)
	s.AnswerStable = s.OKSamples > 0 && len(answerHashes) <= 1
	s.CacheResults = sortedStringCounts(cacheCounts)
	s.Tiers = sortedStringCounts(tierCounts)
	s.StatusCodes = sortedStatusCounts(statusCounts)
	s.FinishReasons = sortedStringCounts(finishReasonCounts)
	s.ResponseModels = sortedStringCounts(responseModelCounts)
	s.Errors = sortedStringCounts(errorCounts)

	return s, nil
}

func readChatStreamResults(path string, visit func(probe.ChatStreamResult)) error {
	f, err := os.Open(path)
	if err != nil {
		return fmt.Errorf("open %s: %w", path, err)
	}
	defer f.Close()

	scanner := bufio.NewScanner(f)
	scanner.Buffer(make([]byte, 0, 64*1024), 8*1024*1024)

	lineNo := 0
	for scanner.Scan() {
		lineNo++
		line := strings.TrimSpace(scanner.Text())
		if line == "" {
			continue
		}

		var r probe.ChatStreamResult
		if err := json.Unmarshal([]byte(line), &r); err != nil {
			return fmt.Errorf("parse %s line %d: %w", path, lineNo, err)
		}

		visit(r)
	}

	if err := scanner.Err(); err != nil {
		return fmt.Errorf("scan %s: %w", path, err)
	}

	return nil
}
